...
export default function Sesija() {
  ...
  const ucitaj = useCallback(async (row) => {
    setSesija(row);
  ...
  if (!sesija) {
    return (
      <div className="stranica mx-auto max-w-md px-5 pt-8 md:max-w-2xl md:px-8">
        ...
        {trainer ? (
          <>
            ...
            <div className="mt-6 space-y-2 md:grid md:grid-cols-2 md:gap-3 md:space-y-0">
              {teme.map((t) => (
                <button key={t.id} onClick={() => pokreni(t.id)} className="ploca w-full p-4 text-left">
    ...
    <div className="ekran px-5 md:px-8">
      <div className="mx-auto w-full max-w-md md:max-w-2xl">
        <div className="flex items-center justify-between pt-4">
          <button onClick={() => router.push("/start")} className="text-sm text-tiho">← Natrag</button>
          {trainer && (
            <button onClick={prekini} className="text-sm text-alarm">Prekini sat</button>
          )}
        </div>
      ...
      {/* Mitte: darf rollen, falls die Karte lang ist */}
